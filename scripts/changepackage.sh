...
[[ "${__usage+x}" ]] || read -r -d '' __usage <<-'EOF' || true # exits non-zero when EOF encountered
  -p --package  [arg] Package to set. Required.
...
[[ "${__helptext+x}" ]] || read -r -d '' __helptext <<-'EOF' || true # exits non-zero when EOF encountered
 This script change applicationId and default package of this android project.
EOF
...
[[ "${arg_p:-}" ]]     || help      "Setting a package with -p or --package is required"
...
### 0 - Go up from script folder
__root_project="$(cd $__dir && cd ".." && pwd)"
info "__root_project: ${__root_project}"

### 1 - Change package in files
find ${__root_project} -type f -not -path '*/\.*' -exec grep -Il '.' {} \; | xargs -L 1 sed -i .tmp -r -e "s/fr.bipi/${arg_p}/g"

### Clean
find ${__root_project} -name "*.tmp" -exec rm "{}" \;
