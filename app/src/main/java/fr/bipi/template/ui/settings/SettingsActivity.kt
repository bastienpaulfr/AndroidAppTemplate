...
import fr.bipi.template.data.repository.settings.KEY_CLEAR_LOG
import fr.bipi.template.data.repository.settings.KEY_EXPORT_LOG
import fr.bipi.template.data.repository.settings.KEY_VERBOSE_DEBUG
import fr.bipi.template.data.repository.settings.KEY_VERSION
import fr.bipi.template.databinding.SettingsActivityBinding
    ...
    class AppFragment : com.takisoft.preferencex.PreferenceFragmentCompat() {
        ...
        override fun onCreatePreferencesFix(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            findPreference<Preference>(KEY_EXPORT_LOG)?.setOnPreferenceClickListener {
                appSettingsViewModel.exportLogs().observe(this) { state ->
                    when (state) {
